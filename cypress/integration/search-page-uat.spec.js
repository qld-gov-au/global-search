...
context('Search page', () => {
  it('Filter should not be visible if scope or profile are not present in the URL', () => {
    ...
    cy.visit(`${ROOT_URL}/?query=rego&collection=${search_collection}&profile=qld`)
    cy
      .get('.qg-filter-by-results')
      .should('not.exist')
  })
  ...
  it("Filter should appear if scope is present, but if its value is not 'qld.gov.au'", () => {
    cy.visit(`${ROOT_URL}/?query=rego&collection=${search_collection}&profile=qld&scope=tmr.qld.gov.au&collection=${search_collection}`)
    cy
      .get('.qg-filter-by-results')
      .should('be.visible')
  })
