            ...
            ${resultPacket.results.map((result: any) => html`
                <li class="qg-search-results__results-list-item">
                    ...
                    <ul class="qg-search-results__results-list">
                        ...
                        <li class="meta"><span class="qg-search-results__url">${result.indexUrl}</span> - <span class="file-size">${formatSize(result.fileSize)}</span> ${result.date ? html`<span>&nbsp;-&nbsp;${formatDate(result.date)}</span>` : ''}</li>
                    </ul>
                </li>`
            )}
        </ul>
